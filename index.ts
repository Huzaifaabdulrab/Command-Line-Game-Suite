...
async function playGame() {
    ...
    let conditionCode: boolean = false;
    let YourCode: string = '';
    ...
    let verifyCond: boolean = false;
    while (!verifyCond) {
        const randomNum2: number = Math.floor(Math.random() * 10000)
    ...
    while (!gameCompleted) {
        if (gamesList.games === "number-Guessing Game") {
            let numGuessCode: number = 1122;
            let gameCodeCondition: boolean = false;
            ...
            let gameCondition: boolean = false;
            while (!gameCondition) {
        ...
        if (gamesList.games === "Star making game") {
            let pyramidGameCode: number = 3344;
            let gameCodeCondition2: boolean = false;
            ...
            while (!gameCodeCondition2) {
                ...
                if (pyramidGame.pGame == pyramidGameCode.toString()) {
                    ...
                    const n: number = parseInt(starGame.pyramidGame);
                    if (!isNaN(n) && n > 0) {
                        let a: string = "";
        ...
        if (gamesList.games === "Head or Tails") {
            let head_tailCode: number = 5566;
            let gameCodeCondition3: boolean = false;
            ...
            let gameCondition3: boolean = false;
